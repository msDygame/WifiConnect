...
/**
 * Parcel-like entity class for VPN profiles. To keep things simple, all fields
 * are package private. Methods are provided for serialization, so storage can
 * be implemented easily. Two rules are set for this class. First, all fields
 * must be kept non-null. Second, always make a copy using clone() before
 * modifying.
 */
class VpnProfile implements Cloneable {
	...
	final String key; // -1
	String name = ""; // 0
	int type = TYPE_L2TP_IPSEC_PSK; // 1
	String server = ""; // 2
	String username = ""; // 3
	String password = ""; // 4
	String dnsServers = ""; // 5
	...
	static VpnProfile decode(String key, byte[] value) {
		try {
			...
			String[] values = new String(value, "UTF-8").split("\0", -1);
			...
			profile.saveLogin = !profile.username.equals("")
					|| !profile.password.equals("");
			return profile;
		} catch (Exception e) {
			// ignore
		}
